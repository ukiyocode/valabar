public class Item : Gtk.FlowBoxChild
{
    ...
    private bool query_tooltip_cb(int x, int y, bool keyboard, Gtk.Tooltip tip)
    {
        tip.set_icon_from_gicon(proxy.tooltip_icon,IconSize.DIALOG);
        tip.set_markup(proxy.tooltip_text);
        return true;
    }
    private unowned SystemTray get_applet()
    {
        return this.get_parent() as SystemTray;
    }
...
//       }
    public bool context_menu()
    {
        ...
        if (proxy.menu != null)
        {
            menu.hide.connect(()=>{(this.get_parent() as Gtk.FlowBox).unselect_child(this);});
